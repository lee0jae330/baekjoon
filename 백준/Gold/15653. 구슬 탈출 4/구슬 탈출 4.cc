...
void bfs() {
	queue<pair< pair<pair<pair<int, int>, pair<int, int>>, int>,string>>q;
	visited[rx][ry][bx][by]=1;
	q.push({{{{rx,ry},{bx,by}},0},""});
	while (!q.empty()) {
		rx = q.front().first.first.first.first;
		ry = q.front().first.first.first.second;
		bx = q.front().first.first.second.first	;
		by = q.front().first.first.second.second;
			...
			if (i == 0) { //위
				if (ry == by) { //둘의 y좌표가 겹쳐있을 때
					if (rx < bx) { //빨간 공이 더 위에 있을때
						...
						if (!visited[tmp_rx][ry][tmp_bx][by] && f!=2 ) {
							if (f) {
								flag=1;
								sol=cnt+1;
								result = s+'U';
							...
							q.push({ { {{tmp_rx, ry},{tmp_bx,by}},cnt + 1 },s+'U' });
						}
					}
						...
						if (!visited[tmp_rx][ry][tmp_bx][by] ) {
							if (f) {
								flag=1;
								sol = cnt + 1;
								result = s + 'U';
					...
					if (!visited[tmp_rx][ry][tmp_bx][by] && !f) {
						if (tmp_flag) {
							flag = 1;
							sol = cnt + 1;
							result = s + 'U';
				...
				if (rx == bx) { //두 공 x좌표가 겹쳐있을 때
					if (ry > by) { // 빨간 공이 더 오른쪽에 있을 때
						...
						if (!visited[rx][tmp_ry][bx][tmp_by] && f!=2) {
							if (f) {
								flag=1;
								sol = cnt + 1;
								result = s+'R';
							...
							q.push({ { {{rx,tmp_ry},{bx,tmp_by}},cnt + 1 } ,s+'R'});
						}
					}
						...
						if (!visited[rx][tmp_ry][bx][tmp_by]) {
							if (f) {
								flag=1;
								sol = cnt + 1;
								result =s+'R';
					...
					if (!visited[rx][tmp_ry][bx][tmp_by] && !f) {
						if (tmp_flag) {
							flag = 1;
							sol = cnt + 1;
							result =s+'R';
				...
				if (ry == by) { //둘의 y좌표가 겹쳐있을 때
					if (rx > bx) { //빨간 공이 더 아래에 있을때
						...
						if (!visited[tmp_rx][ry][tmp_bx][by] && f!=2) {
							...
							visited[tmp_rx][ry][tmp_bx][by] = 1;
							q.push({ { {{tmp_rx, ry},{tmp_bx,by}},cnt + 1 } ,s+'D'});
						}
					}
				...
				if (rx == bx) { //두 공 x좌표가 겹쳐있을 때
					if (ry < by) { // 빨간 공이 더 왼쪽에 있을 때
						...
						if (!visited[rx][tmp_ry][bx][tmp_by] && f!=2) {
							...
							visited[rx][tmp_ry][bx][tmp_by] = 1;
							q.push({ { {{rx,tmp_ry},{bx,tmp_by}},cnt + 1 } ,s+'L'});
						}
					}
